...
const DiffViewer = () => {
  ...
  const files = fileDiffs
    ...
    .filter(
      (file) =>
        isRenameMode ||
        issues.some((i) => normalize(i.file)?.endsWith(normalize(file)))
    );
  ...
  const toggleFileExpand = (filePath) => {
    setOpenFiles((prev) => ({ ...prev, [filePath]: !prev[filePath] }));
  };
  ...
  const applyAll = async () => {
    try {
      setFixLoading(true);
      await axios.post(`http://localhost:9090/api/fix/apply/${scanId}`);
      navigate(`/projects/${projectKey}/summary`);
    } catch {
      setSnackbar({ open: true, message: "AutoFix ALL failed", severity: "error" });
    } finally {
      setFixLoading(false);
    }
  };
  ...
  const handleRenameVariable = async () => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:9090/api/refactor/rename-variable",
        { scanId, oldName: renameOldName, newName: renameNewName }
      );
      ...
      if (mapped.length > 0) {
        ...
        setSnackbar({ open: true, message: "No variable usages found", severity: "info" });
      }
      ...
      setSnackbar({ open: true, message: "Variable rename failed", severity: "error" });
    } finally {
      setFixLoading(false);
    }
  };
  ...
  const handleFixByRule = async (e, ruleId) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to fix all auto-fixable issues for rule ${ruleId}?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (resp.status === 200) {
        if (resp.data.fixesApplied > 0) {
          ...
          alert(
            `No new fixes were applied for rule ${ruleId}. The issues may already be fixed, or they may require manual intervention.`
          );
      ...
      <Box className="diff-viewer-page">
        ...
        <Box className="diff-header-section">
          <Breadcrumbs separator={<ChevronRight size={14} />}>
            <Link onClick={() => navigate("/dashboard")}>Projects</Link>
        ...
        <Box className="review-layout">
            ...
            <div className="issue-list">
              {groupBy === "file" ? (
                ...
                files
                  .filter(
                    (filePath) =>
                      isRenameMode || getFileIssues(filePath).length > 0
                  )
                  .map((filePath) => (
                ...
                /* ── GROUP BY RULE ── */
                Array.from(new Set(issues.map((i) => i.ruleId))).map(
                  (ruleId) => {
                    ...
                    return (
                      <div key={ruleId} className="rule-group">
                        <div
                          ...
                          onClick={() => toggleFileExpand(ruleId)}
                        >
                          ...
                          {isAutoFixable && (
                            <button
                              className={`rule-fix-btn-inline ${
                                isFixingRule[ruleId] ? "fixing" : ""
                              }`}
          ...
          {/* ── END RIGHT PANEL (diff-content) ── */}

        </Box>
        {/* ── END review-layout ── */}

      </Box>
      {/* ── END diff-viewer-page ── */}
