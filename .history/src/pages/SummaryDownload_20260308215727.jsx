...
const scanId = localStorage.getItem("scanId");
  ...
  useEffect(() => {
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        const resultRes = await axios.get(`http://localhost:8080/api/scan/result/${scanId}`)
      

        const reportRes = await axios.get(`http://localhost:8080/api/fix/report/${scanId}`)
  ...
  const handleDownload = () => {
   ...
   window.open(`http://localhost:8080/api/fix/download/${scanId}`)
  ...
  window.open(
    `http://localhost:8080/api/diff/report/download/${scanId}`
  );
