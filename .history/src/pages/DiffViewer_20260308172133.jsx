...
const fetchData = async () => {
  ...
  try {
    ...
    const selected = backendIssues.filter(i =>
      selectedIssueKeys.includes(i.key)
    );

    const issueList = selected.map(issue => ({
      id: issue.key,
      file: issue.filePath,
      title: issue.message,
...
setSelectedFixes(selectedFixMap);
 ...
 }, [projectKey, selectedIssueKeys, scanId]);

  /* ---------------- CURRENT FILE ---------------- */

 const currentFile = fileDiffs.length > 0 ? fileDiffs[currentIndex] : null;
  /* ---------------- FILE GROUP ---------------- */
...
const selected = selectedIssueKeys.length
  ? backendIssues.filter(i => selectedIssueKeys.includes(i.key))
  : backendIssues;
