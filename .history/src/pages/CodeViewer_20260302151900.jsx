...
if (!projectKey) {
 ...
 useEffect(() => {
  ...
  const matchedIssue = issues.find(i => i.id === initialIssueKey);
  ...
  const applySelected = async () => {
    ...
    if (selectedIssues.length === 0) {
      return toast.error("No issues selected");
    }
  ...
  const applyAll = async () => {
    if (issues.length === 0) {
      return toast.error("No issues to apply");
    }
...
useEffect(() => {
  // If coming from Issues page, don't override
  ...
  openFile(firstIssue.file);
  setActiveIssueId(firstIssue.id);

  setTimeout(() => {
    const el = document.getElementById(`line-${firstIssue.line}`);
    if (el) {
      el.scrollIntoView({
        behavior: "smooth",
        block: "center"
              ...
              <div className="file-group" key={filePath}>
                <div
                  className={`file-header ${
                    selectedFile === filePath ? "selected-file" : ""
                  }`}
                  onClick={() => {
